...
void PrintUsage()
{
	printf(
		...
		"\n"
		"Usage:\n"
		"	 ModelImporter.exe [Options] <InputFile> <OutFile>\n"
		"Options:\n"
		"    -scale <number>	Scale to apply to the medol.\n"
		"\n"
	);
}
	...
	if (textureCount > 0)
	{
		...
		if (aiMaterial->GetTexture(textureType, 0, &texturePath) == aiReturn_SUCCESS)
		{
			if (texturePath.C_Str()[0] == '*')
			{
				...
				{
					//ASSERT(embeddedTexture->mHeight == 0, "Error: Unrecognized texture format!");
...
uint32_t TryAddBone(const aiBone* bone, Skeleton& skeleton, BoneIndexLookUp& boneIndexMap)
{
	...
	ASSERT(!name.empty(), "Error: aiBone does not have a name!");

	auto iter = boneIndexMap.find(name);
				...
				if (scene->HasAnimations())
				{
					...
					for (uint32_t animIndex = 0; animIndex < scene->mNumAnimations; ++animIndex)
					{
						...
						if (aiAnimation->mName.length > 0)
						{
							...
							std::replace(animClip.name.begin(), animClip.name.end(), ' ', '_');
						}
						else
						{
					...
					if (animationOnly)
					{
						ModelIO::SaveAnimation(file.path(), *model);
