// Description: Post processing shader for CEngine.
...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	else if (mode == 5) // Vignette
	{
		float4 color = textureMap0.Sample(textureSampler, input.texCoord);
		finalColor = color / 2.0f;
	}
	else if (mode == 6) //Flip Vertically
	{
		float2 texCoord = input.texCoord;
		texCoord.y *= -1;
		finalColor = textureMap0.Sample(textureSampler, texCoord);
	} 
	else if (mode == 7) // Flip Horizontal
	{
		float2 texCoord = input.texCoord;
		texCoord.x *= -1;
		finalColor = textureMap0.Sample(textureSampler, texCoord);
	}
	else if (mode == 8) // Emboss
	{
		finalColor = float4(0.5, 0.5, 0.5, 1.0);
		finalColor -= textureMap0.Sample(textureSampler, input.texCoord - 0.001) * param1;
		finalColor += textureMap0.Sample(textureSampler, input.texCoord + 0.001) * param1;
		finalColor = (finalColor.r + finalColor.g + finalColor.b) / 3.0f;
	}
	else if (mode == 9) // Blur
	{
