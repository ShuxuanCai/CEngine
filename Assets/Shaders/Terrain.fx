// Description: Terrain shader for PCEngine
...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	if (useShadowMap)
	{
		...
		float2 shadowUV = input.lightNDCPosition.xy / input.lightNDCPosition.w; // [-1, 1]
		float u = (shadowUV.x + 1.0f) * 0.5f; // [0, 2] > [0, 1]
		float v = 1.0f - ((shadowUV.y + 1.0f) * 0.5f); // [0, 1]
