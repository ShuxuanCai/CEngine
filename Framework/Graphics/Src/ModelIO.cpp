...
void ModelIO::SaveSkeleton(std::filesystem::path filePath, const Model& model)
{
	...
	uint32_t bonesCount = model.skeleton->bones.size();
	...
	for (uint32_t i = 0; i < bonesCount; ++i)
	{
		...
		fprintf_s(file, "BoneChildrenCount: %u\n", childCount);
...
void ModelIO::LoadSkeleton(std::filesystem::path filePath, Model& model)
{
	...
	model.skeleton->bones.resize(bonesCount);
	...
	for (uint32_t i = 0; i < bonesCount; ++i)
	{
		...
		int childCount = 0;
		fscanf_s(file, "BoneChildrenCount: %u\n", &childCount);
		...
		if (boneIndex == 4)
		{
			Math::Matrix4 rotMat = Math::Matrix4::RotationY(1.72f);
			bone->offsetTransform = rotMat * bone->offsetTransform;
		}
	}
